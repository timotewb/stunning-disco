...
import { Plus, Search, X, Sparkles, ExternalLink } from 'lucide-react';
import type {
  WorkRequest,
  TeamMember,
...
const Requests: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [reqs, t, at, src, typ, pri, sts, eff] = await Promise.all([
        getWorkRequests(),
        getTeam(),
        getAllocationTypes(),
        getRequestSourceConfigs(),
      ...
      <div className="px-6 pt-6 pb-0 flex-shrink-0">
        ...
        {draftCount > 0 && (
          <div className="mb-4 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5">
            ...
            <button
              onClick={() => setFilterDraftsOnly(true)}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((r) => {
                ...
                const isBeingDeleted = deleteConfirm === r.id;
                return (
                  <tr
                    key={r.id}
